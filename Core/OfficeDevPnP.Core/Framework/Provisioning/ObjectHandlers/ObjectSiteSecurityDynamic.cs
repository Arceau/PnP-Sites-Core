...
{
    internal class ObjectSiteSecurityDynamic : ObjectHandlerBase
    {
        public override string Name
        {
            get { return "Dynamic Sub Site Security"; }
        }
                ...
                if (creationInfo.IncludeSiteGroups)
                {
                    web.Context.Load(web.RoleAssignments.Groups,
                        o => o.IncludeWithDefaultProperties(
                            ...
                            gr => gr.Owner.PrincipalType,
                            gr => gr.RequestToJoinLeaveEmailSetting
                            ));

                    web.Context.ExecuteQueryRetry();

                    foreach (var group in web.RoleAssignments.Groups .AsEnumerable().Where(o => !associatedGroupIds.Contains(o.Id)))
                    {
                        ...
                        var siteGroup = new SiteGroup()
                        {
                            ...
                            Owner = ReplaceGroupTokens(web, group.Owner),
                        ...
                        if (webRoleAssignment.Member.LoginName != "Excel Services Viewers")
                        {
                            foreach (var roleDefinition in webRoleAssignment.RoleDefinitionBindings)
                            {
                                if (roleDefinition.RoleTypeKind != RoleType.Guest)
                                {
                                    ...
                                    modelRoleAssignment.Principal = ReplaceGroupTokens(web, webRoleAssignment.Member);
        ...
        private string ReplaceGroupTokens(Web web, Principal principal)
        {
            var parsedPrincipal = principal.LoginName;
            if (!web.AssociatedOwnerGroup.ServerObjectIsNull.Value)
            {
                parsedPrincipal = principal.LoginName.Replace(web.AssociatedOwnerGroup.Title, "{associatedownergroup}");
            }
            if (!web.AssociatedMemberGroup.ServerObjectIsNull.Value)
            {
                parsedPrincipal = principal.LoginName.Replace(web.AssociatedMemberGroup.Title, "{associatedmembergroup}");
            }
            if (!web.AssociatedVisitorGroup.ServerObjectIsNull.Value)
            {
                parsedPrincipal = principal.LoginName.Replace(web.AssociatedVisitorGroup.Title, "{associatedvisitorgroup}");
            }
            if (principal.PrincipalType == PrincipalType.SharePointGroup)
            {
                parsedPrincipal = principal.LoginName.Replace(web.Title, "{sitename}");
            }
        ...
        public override bool WillExtract(Web web, ProvisioningTemplate template, ProvisioningTemplateCreationInformation creationInfo)
        {
            ...
            {
                _willExtract = creationInfo.IncludeSiteGroups && web.IsSubSite();
            }
